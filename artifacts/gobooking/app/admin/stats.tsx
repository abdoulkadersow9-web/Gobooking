...
export default function AdminStats() {
  ...
  return (
    <SafeAreaView style={S.safe}>
      <View style={S.header}>
        <Pressable onPress={() => router.replace("/dashboard/super-admin")} style={S.backBtn} hitSlop={8}>
      ...
      {loading ? (
        ...
        >
          ...
          <StatCard label="Réservations du jour" value={fmt(stats?.bookings_today)} icon="ticket-outline"         color={ACCENT}    />
          <StatCard label="Revenus du jour"       value={fmtF(stats?.revenue_today)} icon="cash-outline"          color="#16a34a"   />
          <StatCard label="Trajets actifs"        value={fmt(stats?.active_trips)}   icon="bus-outline"           color="#2563eb"   />

          <Text style={[S.section, { marginTop: 20 }]}>Global</Text>
          <StatCard label="Utilisateurs"    value={fmt(stats?.total_users)}    icon="people-outline"          color={PURPLE}    />
          <StatCard label="Réservations"    value={fmt(stats?.total_bookings)}  icon="document-text-outline"   color="#475569"   />
          <StatCard label="Colis"           value={fmt(stats?.total_parcels)}   icon="cube-outline"            color="#0891b2"   />
          <StatCard label="Compagnies"      value={fmt(stats?.total_companies)} icon="business-outline"        color="#7c3aed"   />
          <StatCard label="Revenus totaux"  value={fmtF(stats?.total_revenue)}  icon="wallet-outline"          color="#16a34a"   />
        </ScrollView>
      )}
    </SafeAreaView>
  );
...
const S = StyleSheet.create({
  ...
  section:   { fontSize: 13, fontWeight: "700", color: "#6b7280",
