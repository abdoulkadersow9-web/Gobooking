...
export default function TicketsScreen() {
  ...
  /* ── Désélection automatique si le trajet passe en_route ── */
  useEffect(() => {
    if (!selectedTrip) return;
    const updated = trips.find(t => t.id === selectedTrip.id);
    if (updated && (updated.status === "en_route" || updated.status === "en_cours")) {
  ...
  /* ── Trip classification: en route vs boarding vs à venir ── */
  const _now      = new Date();
  ...
  const _yesterday = (() => { const d = new Date(_now); d.setDate(d.getDate()-1); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`; })();

  const _isEnRoute = (t: Trip) => {
    /* Un trajet "en route" est celui dont le statut API est explicitement en_route/en_cours.
       Les trajets dont l'heure est passée mais dont le statut n'est PAS encore "en_route"
       restent dans la liste "À venir" — le guichet peut encore vendre jusqu'au départ officiel. */
    const isRecentDate = t.date === _today || t.date === _yesterday;
    if (!isRecentDate) return false;
    return t.status === "en_route" || t.status === "en_cours";
  };
  const _isBoarding = (t: Trip) => t.status === "boarding" && (t.date === _today || t.date === _yesterday);

  const tripsEnRoute  = trips.filter(t => _isEnRoute(t) && !["completed","cancelled","arrived"].includes(t.status ?? ""));
  const tripsBoarding = trips.filter(t => _isBoarding(t) && !_isEnRoute(t));
  const tripsFuture   = trips.filter(t => !_isEnRoute(t) && !_isBoarding(t) && !["completed","cancelled","arrived"].includes(t.status ?? ""));
          ...
          <View style={S.card}>
            ...
            {loadingTrips ? (
              ...
              <View style={{ gap: 4 }}>
                ...
                {/* ══ Section A : PARTIS — Vente guichet fermée ══ */}
                {tripsEnRoute.length > 0 && (
                  <>
                    <View style={{
                      ...
                      borderWidth: 1, borderColor: "#CBD5E1", marginBottom: 4,
                    }}>
                      <Ionicons name="navigate" size={13} color="#64748B" />
                      <Text style={{ fontSize: 12, fontWeight: "800", color: "#475569", letterSpacing: 0.5, flex: 1 }}>
                        PARTIS — Vente guichet fermée
                      </Text>
                      <View style={{ backgroundColor: "#94A3B8", borderRadius: 6, paddingHorizontal: 7, paddingVertical: 2 }}>
                      ...
                      {tripsEnRoute.map(trip => {
                        ...
                        return (
                          <View key={trip.id} style={[S.tripCard, {
                            borderLeftWidth: 4, borderLeftColor: "#94A3B8",
                            opacity: 0.82,
                          }]}>
                            <View style={{ flexDirection: "row", alignItems: "flex-start" }}>
                              <View style={{ flex: 1, gap: 4 }}>
                                <Text style={[S.tripCardRoute, { color: "#64748B" }]} numberOfLines={1}>
                                  {trip.from}{"  "}<Text style={{ color: "#CBD5E1", fontSize: 14 }}>→</Text>{"  "}{trip.to}
                                </Text>
                                <View style={{ flexDirection: "row", alignItems: "center", gap: 5 }}>
                                  <Ionicons name="time-outline" size={13} color="#94A3B8" />
                                  <Text style={[S.tripCardTime, { color: "#94A3B8" }]}>{trip.departureTime} · {trip.date}</Text>
                                </View>
                              </View>
                              <View style={{ alignItems: "flex-end", marginLeft: 12 }}>
                                <Text style={[S.tripCardPrice, { color: "#94A3B8" }]}>{trip.price?.toLocaleString()}</Text>
                                <Text style={[S.tripCardFcfa, { color: "#CBD5E1" }]}>FCFA</Text>
                              </View>
                            </View>
                            <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 8 }}>
                              <View style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#E2E8F0", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 }}>
                                <Ionicons name="navigate-outline" size={10} color="#64748B" />
                                <Text style={{ fontSize: 10, fontWeight: "700", color: "#64748B" }}>En route</Text>
                              </View>
                              {hasOnline ? (
                                <View style={S.tripBadgeBlue}>
                                  <Ionicons name="wifi-outline" size={11} color="#1D4ED8" />
                                  <Text style={S.tripBadgeBlueTxt}>{trip.onlineSeats} places en ligne</Text>
                                </View>
                              ) : null}
                              <View style={{ flex: 1 }} />
                              <Text style={{ fontSize: 10, color: "#94A3B8", fontStyle: "italic" }}>Guichet fermé</Text>
                            </View>
                          </View>
                        );
                      })}
                    ...
                    <View style={{
                      ...
                      <Text style={{ fontSize: 12, color: "#1D4ED8", flex: 1, lineHeight: 17 }}>
                        Les cars partis ne sont plus accessibles à la vente guichet. Les passagers peuvent réserver en ligne uniquement.
                      </Text>
                    </View>
                  </>
                )}

                {/* ══ Section B : En EMBARQUEMENT ══ */}
                {tripsBoarding.length > 0 && (
                  <>
                    <View style={{
                      flexDirection: "row", alignItems: "center", gap: 8,
                      ...
                      paddingHorizontal: 12, paddingVertical: 9,
                      borderWidth: 1.5, borderColor: "#FCD34D", marginBottom: 4,
                      marginTop: tripsEnRoute.length > 0 ? 4 : 0,
                    }}>
                      <Ionicons name="bus" size={13} color="#D97706" />
                      <Text style={{ fontSize: 12, fontWeight: "800", color: "#92400E", letterSpacing: 0.5, flex: 1 }}>
                        EN EMBARQUEMENT — Passagers à bord
                      </Text>
                      <View style={{ backgroundColor: "#D97706", borderRadius: 6, paddingHorizontal: 7, paddingVertical: 2 }}>
                        <Text style={{ fontSize: 10, fontWeight: "800", color: "#fff" }}>{tripsBoarding.length}</Text>
                      </View>
                    </View>
                ...
                {tripsFuture.length > 0 && (
                  <>
                    ...
                    <View style={{
                      ...
                      marginTop: (tripsEnRoute.length > 0 || tripsBoarding.length > 0) ? 4 : 0,
                    }}>
...
const S = StyleSheet.create({
  ...
  card:      { backgroundColor: "#fff", borderRadius: 18, padding: 16, elevation: 4, shadowColor: "#D97706", shadowOpacity: 0.09, shadowRadius: 14, shadowOffset: { width: 0, height: 5 }, gap: 10, borderWidth: 1, borderColor: "#FEF3C7" },
  cardHeader:{ flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 4 },
  cardTitle: { fontSize: 15, fontWeight: "700", color: "#111827", letterSpacing: -0.2 },

  tripCard:       { borderWidth: 1.5, borderColor: "#E5E7EB", borderRadius: 16, padding: 16, backgroundColor: "#fff", overflow: "hidden", shadowColor: "#000", shadowOpacity: 0.04, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
  tripCardSel:    { borderColor: G, backgroundColor: "#FFFBEB", borderWidth: 2, shadowColor: G, shadowOpacity: 0.12, elevation: 4 },
  tripCardRoute:  { fontSize: 16, fontWeight: "800", color: "#111827", letterSpacing: -0.3 },
  tripCardTime:   { fontSize: 12, color: "#6B7280", fontWeight: "500" },
  tripCardPrice:  { fontSize: 22, fontWeight: "800", color: "#F97316", letterSpacing: -0.5 },
  tripCardFcfa:   { fontSize: 11, color: "#9CA3AF", fontWeight: "600", textAlign: "right" },
  tripCardDivider:{ height: 1, backgroundColor: "#FEF3C7", marginVertical: 14 },
  tripBadgeGreen: { flexDirection: "row" as const, alignItems: "center" as const, gap: 4, backgroundColor: G_LIGHT, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 5 },
  tripBadgeGreenTxt: { fontSize: 11, fontWeight: "700", color: G_DARK },
  tripBadgeBlue:  { flexDirection: "row" as const, alignItems: "center" as const, gap: 4, backgroundColor: "#EFF6FF", borderRadius: 8, paddingHorizontal: 8, paddingVertical: 5 },
  tripBadgeBlueTxt: { fontSize: 11, fontWeight: "600", color: "#1D4ED8" },
  tripSelIcon:    { width: 26, height: 26, borderRadius: 13, borderWidth: 1.5, borderColor: "#E5E7EB", alignItems: "center", justifyContent: "center", backgroundColor: "#F9FAFB" },
  tripSelIconActive: { borderColor: G, backgroundColor: G },
  tripSeatPlanBtn:{ flexDirection: "row" as const, alignItems: "center" as const, justifyContent: "center" as const, gap: 8, backgroundColor: G, borderRadius: 10, paddingVertical: 12, shadowColor: G, shadowOpacity: 0.3, shadowRadius: 6, elevation: 3 },
  ...
  recap:     { backgroundColor: "#FFFBEB", borderRadius: 18, padding: 18, borderWidth: 2, borderColor: "#FDE68A", gap: 10, shadowColor: "#D97706", shadowOpacity: 0.08, shadowRadius: 10, shadowOffset: { width: 0, height: 3 }, elevation: 3 },
  recapTitle:{ fontSize: 15, fontWeight: "800", color: G_DARK, marginBottom: 4, letterSpacing: -0.2 },
  recapRow:  { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 2 },
  recapKey:  { fontSize: 13, color: "#6B7280", fontWeight: "500" },
  recapVal:  { fontSize: 13, fontWeight: "700", color: "#111827" },

  submitBtn:  { backgroundColor: G, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, padding: 17, borderRadius: 16, elevation: 5, shadowColor: G, shadowOpacity: 0.35, shadowRadius: 12, shadowOffset: { width: 0, height: 5 } },
  submitTxt:  { color: "#fff", fontSize: 16, fontWeight: "800", letterSpacing: 0.2 },
  ...
  ticketCard: { width: "100%", backgroundColor: "#FFFBEB", borderRadius: 20, padding: 18, borderWidth: 2, borderColor: "#FDE68A", marginTop: 16, gap: 6, shadowColor: "#D97706", shadowOpacity: 0.14, shadowRadius: 16, shadowOffset: { width: 0, height: 6 }, elevation: 6 },
  ticketCardHeader: { borderBottomWidth: 1.5, borderColor: "#FDE68A", paddingBottom: 12, marginBottom: 8, alignItems: "center" },
  ticketRef:  { fontSize: 17, fontWeight: "800", color: G, letterSpacing: 1.5 },
  ticketRoute:{ fontSize: 16, fontWeight: "800", color: "#111827", textAlign: "center", marginBottom: 8 },
  ticketRow:  { flexDirection: "row", justifyContent: "space-between", paddingVertical: 5, borderBottomWidth: 1, borderColor: "#FEF3C7" },
  ticketKey:  { fontSize: 12, color: "#6B7280", fontWeight: "500" },
  ticketVal:  { fontSize: 12, fontWeight: "700", color: "#111827" },
  ticketTotalRow: { borderTopWidth: 2, borderColor: "#FDE68A", borderBottomWidth: 0, paddingTop: 12, marginTop: 6 },
  ticketTotalKey: { fontSize: 14, fontWeight: "800", color: G_DARK },
  ticketTotalVal: { fontSize: 17, fontWeight: "800", color: "#F97316" },

  printBtn:    { marginTop: 20, backgroundColor: G_DARK, flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 32, paddingVertical: 15, borderRadius: 16, width: "100%", justifyContent: "center", shadowColor: G_DARK, shadowOpacity: 0.22, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 5 },
  printBtnTxt: { color: "#fff", fontSize: 15, fontWeight: "800" },
  newSaleBtn:    { marginTop: 10, backgroundColor: G, flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 32, paddingVertical: 15, borderRadius: 16, width: "100%", justifyContent: "center", shadowColor: G, shadowOpacity: 0.28, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 5 },
  newSaleBtnTxt: { color: "#fff", fontWeight: "800", fontSize: 15 },
