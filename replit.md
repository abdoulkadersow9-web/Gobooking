...
- **Badges dynamiques Home Agent (Phase 3)**: `GET /agent/home/badges` retourne `colisAValider`/`reservationsEnAttente`/`alertesActives`. Queries sur `parcels.company_id` (pas agency_id), `bookings JOIN trips` via `trips.company_id`, `agent_alerts.company_id`. Poll toutes les 45s dans `home.tsx`. Badges rouges positionnés sur icône (top:-2, right:-2) + texte "X en attente" sous le module label. Design `rapport.tsx` modernisé: type chips en grille 2 colonnes avec icônes, indicateur caractères dynamique (rouge/vert) avec badge "X / 10 min".
- **Multi-compagnies & isolation des données (Phase 2)**: 5 compagnies de simulation créées dans la DB: UTB Express (utb-co, admin@utbexpress.ci/utb1234), ETV Express (etv-co, admin@etvexpress.ci/etv1234), Sama Transport (sama-co, admin@samatransport.ci/sama1234), CA Trans Transport (ca-co, admin@catrans.ci/ca1234), SMT Transport (smt-co, admin@smttransport.ci/smt1234). Chaque compagnie a: ses propres agences (17 total), agents (18 total), bus (24 total), trajets (20 total avec sièges), réservations et colis. Isolation renforcée dans l'API: `/company/stats` filtre bookings+parcels par companyId (corrigé: fetch global précédent), `/agent/trips` et `/trips/today` exigent un companyId (plus de fallback global), `/agent/boarding` filtre par company si pas de tripId, `/agent/suivi/overview` et `/logistique/overview` utilisent l'agentRecord.companyId (corrigé: user.companyId inexistant sur users table). Toutes les créations de réservations (agent guichet + company) ajoutent maintenant le companyId au booking. Lien getOrCreateCompany fonctionne via email match entre users.email et companies.email.
